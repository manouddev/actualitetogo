...
const state = () => ({
  ...
  const actions = {
    async sendOtpForgotPassword({ commit }, payload) {
        const response = await axios.post("/api/send_otp/forgot_password", payload).catch((err) => { console.log(err);});
    ...
    async checkOtpForgotPassword({ commit }, payload) {
        const response = await axios.patch("/api/check_otp/forgot_password", payload).catch((err) => { console.log(err);});
        if (response && (response.data.status == 200)) {
    ...
    async newPassOtpForgotPassword({ commit }, payload) {
        const response = await axios.patch("/api/new_pass/forgot_password", payload).catch((err) => { console.log(err);});
        if (response && (response.data.status == 200)) {
