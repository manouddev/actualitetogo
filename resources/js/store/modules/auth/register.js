...
const state = () => ({
  ...
  const actions = {
    async sendOtpRegister({ commit }, payload) {
        const response = await axios.post("/api/send_otp/register", payload).catch((err) => { console.log(err);});
    ...
    async checkOtpRegister({ commit }, payload) {
        const response = await axios.patch("/api/check_otp/register", payload).catch((err) => { console.log(err);});
        if (response && (response.data.status == 200)) {
    ...
    async newInfoOtpRegister ({ commit }, payload) {
        const response = await axios.put("/api/new_info/register", payload).catch((err) => { console.log(err);});
        if (response && (response.data.status == 200)) {
    ...
    async newPassOtpRegister({ commit }, payload) {
        const response = await axios.patch("/api/new_pass/register", payload).catch((err) => { console.log(err);});
        if (response && (response.data.status == 200)) {
