...
const state = () => ({
    ...
    infosSearchByUserListStatus:null,
    infosSearchByUserListMessage:null,
    infosSearchByUserListData:[],
...
const getters = {
    ...
    getInfosSearchByUserListStatus(state){
        return state.infosSearchByUserListStatus;
    },

    getInfosSearchByUserListMessage(state){
        return state.infosSearchByUserListMessage;
    },

    getInfosSearchByUserListData(state){
        return state.infosSearchByUserListData;
    },
...
const actions = {
    async tagsListDataRequest({ commit }, payload) {
        const response = await jwtInterceptor.get(`backoffice/admin/tags_list?page=${payload.page}&search=${payload.search}`).catch((err) => { console.log(err);});
    ...
    async tagsListHDataRequest({ commit }, payload) {
        const response = await jwtInterceptor.get(`backoffice/admin/tags_search_by_date_infos?page=${payload.page}&searchH=${payload.searchH}`).catch((err) => { console.log(err);});
        if (response && (response.data.data.status == 200)) {
            commit("setInfosTagsListStatus", "success");
            commit("setInfosTagsListMessage", response.data.message);
            commit("setInfosTagsListData", response.data.data);
    ...
    async tagsListHHDataRequest({ commit }, payload) {
        const response = await jwtInterceptor.get(`backoffice/admin/tags_search_by_user_infos?page=${payload.page}&user_id=${payload.user_id}`).catch((err) => { console.log(err);});
        if (response && (response.data.data.status == 200)) {
            commit("setInfosTagsListStatus", "success");
            commit("setInfosTagsListMessage", response.data.message);
            commit("setInfosTagsListData", response.data.data);
    ...
    async searchByDateListDataRequest({ commit }, payload) {
        const response = await jwtInterceptor.get(`backoffice/admin/tags_search_by_date`).catch((err) => { console.log(err);});
        if (response && (response.data.data.status == 200)) {
    ...
    async searchByUserListDataRequest({ commit }, payload) {
        const response = await jwtInterceptor.get(`backoffice/admin/tags_search_by_user`).catch((err) => { console.log(err);});
        if (response && (response.data.data.status == 200)) {
            commit("setInfosSearchByUserListStatus", "success");
            commit("setInfosSearchByUserListMessage", response.data.message);
            commit("setInfosSearchByUserListData", response.data.data);
        }else if(response.data.data.status == 401) {
            commit("setInfosSearchByUserListStatus", "empty");
            commit("setInfosSearchByUserListData", response.data.data);
            commit("setInfosSearchByUserListMessage", response.data.message);
        }else if(response.data.data.status == 422){
            commit("setInfosSearchByUserListStatus", "error");
            commit("setInfosSearchByUserListMessage", response.data.message);
        }
    },

    async tagsShowDataRequest({ commit }, payload) {
        const response = await jwtInterceptor.get("backoffice/admin/tags_show", payload).catch((err) => { console.log(err);});
        if (response && (response.data.data.status == 200)) {
    ...
    async tagsStoreDataRequest({ commit },  payload) {
        const response = await jwtInterceptor.post("backoffice/admin/tags_store", payload).catch((err) => { console.log(err);});
        if (response && (response.data.data.status == 200)) {
    ...
    async tagsUpdateDataRequest({ commit },  payload) {
        const response = await jwtInterceptor.put(`backoffice/admin/${payload.slug}/tags_update`, payload).catch((err) => { console.log(err);});
        if (response && (response.data.data.status == 200)) {
    ...
    async tagsDeleteDataRequest({ commit }, payload) {
        const response = await jwtInterceptor.delete(`backoffice/admin/${payload.slug}/tags_delete`).catch((err) => { console.log(err);});
        if (response && (response.data.data.status == 200)) {
...
const mutations = {
    ...
    setInfosSearchByUserListStatus(state, value){
        state.infosSearchByUserListStatus = value;
    },

    setInfosSearchByUserListMessage(state, value){
        state.infosSearchByUserListMessage = value;
    },

    setInfosSearchByUserListData(state, value){
        state.infosSearchByUserListData = value;
    },
